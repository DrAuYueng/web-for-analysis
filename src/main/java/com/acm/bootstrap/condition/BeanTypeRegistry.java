 ...
 * {@link ListableBeanFactory}. Provides similar functionality to
 * {@link ListableBeanFactory#getBeanNamesForType(Class, boolean, boolean)} but
 * is optimized for use by {@link OnBeanCondition} based on the following
 * assumptions:
 * <ul>
...
abstract class BeanTypeRegistry {
     ...
     * {@code getObjectType} in the case of FactoryBeans. Will include
     * singletons but not cause early bean initialization.
     * 
     * @param type the class or interface to match (must not be {@code null})
     * @return the names of beans (or objects created by FactoryBeans) matching
     *         the given object type (including subclasses), or an empty set if
     *         none
     */
    public abstract Set<String> getNamesForType(Class<?> type);

    /**
     * Attempt to guess the type that a {@link FactoryBean} will return based on
     * the generics in its method signature.
     * 
     ...
     */
    protected final Class<?> getFactoryBeanGeneric(ConfigurableListableBeanFactory beanFactory, BeanDefinition definition,
            String name) {
        try {
    ...
    private Class<?> getConfigurationClassFactoryBeanGeneric(ConfigurableListableBeanFactory beanFactory,
        ...
        Method method = ReflectionUtils.findMethod(factoryClass, definition.getFactoryMethodName());
    ...
    /**
     * Factory method to get the {@link BeanTypeRegistry} for a given
     * {@link BeanFactory}.
     * 
     * @param beanFactory the source bean factory
    ...
    static class DefaultBeanTypeRegistry extends BeanTypeRegistry {
        ...
        public Set<String> getNamesForType(Class<?> type) {
            ...
            if (this.beanFactory instanceof ConfigurableListableBeanFactory) {
                collectBeanNamesForTypeFromFactoryBeans(result, (ConfigurableListableBeanFactory) this.beanFactory, type);
            }
            return result;
        }

        private void collectBeanNamesForTypeFromFactoryBeans(Set<String> result, ConfigurableListableBeanFactory beanFactory,
                Class<?> type) {
    ...
    static class OptimizedBeanTypeRegistry extends BeanTypeRegistry implements SmartInitializingSingleton {
        ...
        private boolean requiresEagerInit(String factoryBeanName) {
            return (factoryBeanName != null && this.beanFactory.isFactoryBean(factoryBeanName) && !this.beanFactory
                    .containsSingleton(factoryBeanName));
        }

        /**
         * Returns the {@link OptimizedBeanTypeRegistry} for the given bean
         * factory.
         * 
        ...
        public static OptimizedBeanTypeRegistry getFromFactory(DefaultListableBeanFactory factory) {
            if (!factory.containsLocalBean(BEAN_NAME)) {
                BeanDefinition bd = new RootBeanDefinition(OptimizedBeanTypeRegistry.class);
